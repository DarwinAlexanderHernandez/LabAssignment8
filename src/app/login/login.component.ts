...
export class LoginComponent implements OnInit {
  ...
  login(user: IUser) {
    ...
    const defaultUser: IUser = { username: 'DHernandez', password: 'Darwin777' };
    if (user.username !== '' && user.password !== '') {
      if (user.username === defaultUser.username && user.password === defaultUser.password) {
        ...
        this.toastService.showToast('danger', 'login failed! Please check your username or password', 5000);
      }
    }
  }
}
